...
namespace BionicFileExplorer.Net
{
  public class SpecialDirectory : Directory
  {
    protected SpecialDirectory(IDirectory parentFolder) : base(null, parentFolder)
    {
      this.IsSpecial = true;
      this.IsDrive = false;
      this.IsNavigationalDirectory = true;
      this.ParentFileSystemItem = parentFolder;
    }
  ...
  public class Directory : FileSystemItemModel, IDirectory
  {
    ...
    public Directory(DirectoryInfo directoryInfo, IDirectory parentDirectoryItem) : base(directoryInfo, parentDirectoryItem)
    {
      ...
      this.ChildFileSystemItems = new ObservableCollection<IFileSystemItemModel>();
      Application.Current.Dispatcher.InvokeAsync(() =>
      {
        BindingOperations.EnableCollectionSynchronization(this.ChildFileSystemItems, this.SyncObject);
      }, System.Windows.Threading.DispatcherPriority.Normal);
    }

    public void ApplyActionOnSubTree(Action<IFileSystemItemModel> action, Predicate<IFileSystemItemModel> predicate)
    {
      foreach (IFileSystemItemModel childItem in this.ChildFileSystemItems)
      {
    ...
    public IEnumerable<IDirectory> GetSubdirectories() => this.ChildFileSystemItems.OfType<IDirectory>();

    public IEnumerable<IFile> GetFiles() => this.ChildFileSystemItems.OfType<IFile>();

    public void Sort(Comparison<IFileSystemItemModel> comparison) => (CollectionViewSource.GetDefaultView(this.ChildFileSystemItems) as ListCollectionView).CustomSort = Comparer<IFileSystemItemModel>.Create(comparison);

    public void Sort() => (CollectionViewSource.GetDefaultView(this.ChildFileSystemItems) as ListCollectionView).CustomSort = this;
    ...
    public bool IsRealized => this.ChildFileSystemItems.Any();
